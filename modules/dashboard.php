  ...
  <div class="panel-body">
		<?php
			...
			for($i = 1; $i < 8; $i++) {
				...
				$query_subjectPending = "SELECT course.name, course.id from course
...
<div class="panel panel-primary">
  ...
  <div class="panel-body">
		<?php
			...
			$query_subject = "SELECT course.name, course.id from course INNER JOIN user_course WHERE user_course.id = course.id AND user_course.uid = {$_SESSION['uid']}  ORDER BY course.name";
...
<div class="panel panel-success">
  ...
  <div class="panel-body">
    <p><i class="fa fa-book"></i> <a href="index.php?page=studentinfo"> <strong><?php print $noOfSubject; ?></strong> Subjects </a></p>
		<?php
			$studentQuery = "SELECT COUNT(DISTINCT sid) as student_count FROM `user_course` INNER JOIN student_course WHERE user_course.id = student_course.id AND user_course.uid = $userId";
